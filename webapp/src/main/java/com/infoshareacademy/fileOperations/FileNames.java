package com.infoshareacademy.fileOperations;

public class FileNames {

    private static final String ROOT_PATH = "/home/kamila/projectsKamila/jjdz8-dream-team/webapp/target/resources/";

    public static final String SUBJECTS_JSON = ROOT_PATH + "subjects.json";

    public static final String USERS_JSON = ROOT_PATH + "users.json";

    public static final String TEACHERS_JSON = "resources/teachers.json";

    public static final String STUDENTS_JSON = ROOT_PATH + "students.json";

    public static final String GRADES_JSON = ROOT_PATH + "grades.json";
}
